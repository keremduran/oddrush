<template>
  <b-container class="game" fluid>
    <b-modal lazy
      ...
      variant="dark"
      size="sm"
      hide-footer no-close-on-backdrop centered>
          ...
          <b-col class="col-xs-12" v-if="dropCutActive">
            <b-row class="dropcut-buttons" align-h="start">
              <b-btn 
                ...
                :style="{backgroundColor: denoms[selectedDenom].color, 
                color: denoms[selectedDenom].textColor}" @click="dropCut(n)" class="dropcut-btn" 
                :key="n+'+'+denoms[selectedDenom].color" >{{ n }}</b-btn>
            </b-row>
...
export default {
  ...
  methods: {
    ...
    round(number){
      ...
      if(this.odd.value === 0.05 || this.odd.value === 0.95 && number % 1 > 0) {
        if(number % 1 <= .25){
